...
void speed() {
	...
	if((previous_proximity - proximity) <= P1) esc.writeMicroseconds(S2); //allow car to line up with the next point
		...
		if(wpr_count>3 && wpr_count<=5) esc.writeMicroseconds(S3);
		else esc.writeMicroseconds(S4); //go wide open 200 works well for me. 
	}
...
void setup() {
	...
	//2 IS IN THE SPEED SECTION WHICH MAKES THE CAR STAY SLOW FOR THE FIRST SEVERAL WAYPOINTS AND THEN GO FAST
}
