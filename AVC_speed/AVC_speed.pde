...
void speed() {
	...
	if((previous_proximity - proximity) <= P1) esc.writeMicroseconds(S2); //allow car to line up with the next point
		...
		if (speed_cur < BREAKING_SPEED)  esc.writeMicroseconds(S2);  // less than 8000 means high speed, apply brakes
		else esc.writeMicroseconds(S2);  //once speed is low enough, resume normal slow-down
	}
	else if(proximity >= P3) esc.writeMicroseconds(S2); //go wide open 200 works well for me. 
}
